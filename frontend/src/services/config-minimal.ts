...
export class ConfigurationService {
  ...
  getConfiguration(_userId: string, _options?: any): Promise<UIConfiguration> {
    return this.getUIConfiguration();
  }
  setConfiguration(_userId: string, config: UIConfigurationUpdate, _options?: any): Promise<UIConfiguration> {
    return this.updateUIConfiguration(config);
  }
  getTheme(): string { return 'light'; }
  setTheme(_theme: string): void {}
  getLanguage(_userId: string): Promise<string> { return Promise.resolve('en'); }
  setLanguage(_userId: string): void {}
}
...
export const isFeatureEnabled = (_featureName: string): boolean => false;
export const getFeatureValue = (_featureName: string): any => false;
export const getSafeAreaValue = (_side: string): number => 0;
